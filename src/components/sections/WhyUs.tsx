const items = [
  ...
  },
  {
    ...
    desc: 'We send clear updates, surface blockers proactively, and never leave you guessing at handover. You know exactly where your project stands at every point.',
  },
  {
    num: '03',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {items.map((item) => (
          ...
          >
            ...
            <div
              aria-hidden
              className="absolute -right-3 -top-4 text-8xl font-bold text-zinc-100 dark:text-zinc-800 select-none pointer-events-none leading-none transition-colors group-hover:text-brand-50 dark:group-hover:text-brand-900/50"
            >
              {item.num}
            </div>
            <div className="relative">
              <span className="text-xs font-bold text-brand-600 dark:text-brand-500 block mb-3 tracking-widest">
                {item.num}
              </span>
